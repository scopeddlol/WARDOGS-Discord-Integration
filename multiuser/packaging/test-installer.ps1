param([string]$Installer = 'dist\WARDOGS-Agent-Setup.exe')
...
$env:WARDOGS_AGENT_DATA_DIR = Join-Path $testRoot 'data'
$env:QT_QPA_PLATFORM = 'offscreen'
# Prevent the developer's external OCR override from hiding a broken bundle.
Remove-Item Env:WARDOGS_AGENT_TESSERACT -ErrorAction SilentlyContinue
...
$installArgs = @('/VERYSILENT', '/SUPPRESSMSGBOXES', '/NORESTART', '/SP-', '/TASKS=""', '/GROUP="WARDOGS Agent Test"', ('/DIR="' + $installPath + '"'))
try {
    Run-Checked $installerPath $installArgs
    Run-Checked (Join-Path $installPath 'WARDOGS Agent.exe') @('--smoke-test', ('"' + (Join-Path $testRoot 'first-launch') + '"'))
    ...
    $sentinel = Join-Path $env:WARDOGS_AGENT_DATA_DIR 'upgrade-check.txt'
    Set-Content $sentinel 'preserve user data'
    Run-Checked $installerPath $installArgs
    if ((Get-Content $sentinel) -ne 'preserve user data') { throw 'Upgrade lost per-user data.' }
    Run-Checked (Join-Path $installPath 'WARDOGS Agent.exe') @('--smoke-test', ('"' + (Join-Path $testRoot 'after-upgrade') + '"'))
...
if (Test-Path (Join-Path $installPath 'WARDOGS Agent.exe')) { throw 'Uninstall left the application executable.' }
