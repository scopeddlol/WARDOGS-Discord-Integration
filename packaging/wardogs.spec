...
datas = [(str(root / 'tray_icon.png'), '.'), (str(root / 'LICENSE'), 'licenses'),
...
pyz = PYZ(a.pure)
exe = EXE(pyz, a.scripts, [], exclude_binaries=True, name='WARDOGS Discord',
          console=False, icon=str(root / 'packaging' / 'app.ico'))
coll = COLLECT(exe, a.binaries, a.datas, strip=False, upx=False, name='WARDOGS Discord')
